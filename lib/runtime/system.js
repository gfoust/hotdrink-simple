...
function ConstraintSystem() {
  ...
  this.allNodes= Object.create( this.baseNodes );

  // Solution graph
  this.sgraph= new Digraph( this.allNodes );
  this.sgraph.selected= {};

  // Enablement graphs
  this.ngraph= new Digraph( this.baseNodes );
...
function addVariable( vv ) {
  ...
  this.allNodes[scc]= scc;
  this.allNodes[smm]= smm;
...
m4_member(ConstraintSystem, addOutput)
function addOutput( vv ) {
  this.addVariable( vv );
  this.outputVars.push( vv );
}
...
function addMethod( mm ) {
  ...
  mm.inputVars.forEach( function( vv ) {
    this.ngraph.addEdge( vv, mm );
  }, this );

  mm.outputVars.forEach( function( vv ) {
    this.ngraph.addEdge( mm, vv );
  }, this );
...
function onPriorityNext( priority, vv ) {
  var sid= stayId( vv );
  this.allNodes[sid].strength= priority;
...
function solve( unenforcedConstraints ) {
  ...
  for (var key in solver.changes) {
    ...
    if (oldmm) {
      oldmm.inputVars.forEach( function( vv ) {
        if (! this.ngraph.hasEdge( vv, oldmm )) {
          this.ngraph.addEdge( vv, oldmm );
          this.scheduleEnableCheck();
        }
      }, this );
    }
...
m4_member(ConstraintSystem, setRelevance)
function setRelevance( vv, mm, isUsed ) {
  if (this.ngraph.hasEdge( vv, mm )) {
    if (! isUsed) {
      this.ngraph.removeEdge( vv, mm );
      this.scheduleEnableCheck();
    }
  }
  else {
...
// Allows multiple changes to result in only a single update
m4_member(ConstraintSystem, scheduleEnableCheck)
function scheduleEnableCheck() {
  if (this.enableTask === null) {
    this.enableTask= setTimeout( this.enableCheck.bind( this ), 0 );
  }
}

// Perform actual update
m4_member(ConstraintSystem, enableCheck)
function enableCheck() {
  LOG(SYS, 'enable')
  if (this.enableTask) {
    clearTimeout( this.enableTask );
    this.enableTask= null;
  }

  var enabledSet= {};
  this.ngraph.nodesUpstreamSameType( this.outputVars )
    .forEach( insertInto, enabledSet );

  this.ngraph.nodes().filter( isType, Variable )
    .forEach( function( vv ) {
      if (vv.disabled.get() && enabledSet[vv]) {
        vv.disabled.set( false );
      }
      else if (! vv.disabled.get() && ! enabledSet[vv]) {
        vv.disabled.set( true );
      }
    }, this );
}

function insertInto( id ) {
  this[id]= true;
}