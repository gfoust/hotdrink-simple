...
function Variable( name, init ) {
  ...
  var id= (name ? name : '') + '#' + vartag + (++varcount);
  Node.call( this, id );
  ...
  this.disabled= new ObservableValue( this, 'disabled', false );
 ...
 */
m4_member(Variable, addSubscriber)
function addSubscriber( obsver ) {
  return this.value.addSubscriber( obsver );
}
