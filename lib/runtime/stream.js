...
/*--------------------------------------------------------------------
 */
m4_member(MethodStream, subscribeToInput)
function subscribeToInput( name ) {
...
function prepareOutputs() {
  setIntersect( this.mm.outputs, this.mm.inputs )
    .forEach( this.subscribeToInput, this );
...
function runAsync() {
  ...
  this.system.pool.schedule( this,
                             this.mm.fn,
                             // must copy the copy because
                             // (1) job may be queued if we run out of workers
                             // (2) we may make changes to the copy
                             //     if input promises resolve
                             shallowCopy( this.contextCopy ),
                             this.nextVersion(),
                             this.pendingInputCount == 0
                           )
}
