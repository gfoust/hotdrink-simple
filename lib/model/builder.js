...
function method(/* name?, signature, fn, context?, async? */) {

  // Interpret arguments
  var name, signature, fn, context, async;
  ...
  var signature= this.parseSignature( signature );
  if (! signature) {
    window.console.error( 'Invalid signature for factory method "method": ' + origSignature );
    return this;
  }
  ...
  for (var key in this._model) {
    if ((signature.inputs.indexOf( this._model[key] ) != -1 ||
         signature.outputs.indexOf( this._model[key] ) != -1  ) && ! (key in context))
    {
  ...
  // Create method
  var mm= new Method( name, signature.inputs, signature.outputs, fn, context, async );
  cc.addMethod( mm );
...
function parseSignature( signature ) {
  ...
  var inputs= leftRight[0].split( /\s*,\s*/ );
  var outputs= leftRight[1].split( /\s*,\s*/ );

  inputs= inputs.map( this.mapToVariable, this );
  outputs= outputs.map( this.mapToVariable, this );

  if (inputs.some( isNotType, Variable) || outputs.some( isNotType, Variable )) {
    return undefined;
  }
