...
function addMethod( mm ) {
  ...
  mm.inputs.forEach( this.addVariable, this );
  mm.outputs.forEach( this.addVariable, this );
  return this;
}
...
function Method( name, inputs, outputs, fn, context, async ) {
  // In order to be used as a node we need a unique id
...
function StayConstraint( vv ) {
  ...
  this.variables= [vv];
  var method= new Method( 'noop', [], [vv], noop );
  this.methods= [method];
