 ...
 * Method streams
 *
 * TODO: when a method returns a promise and then later fulfills it,
 *       the fulfilling should /schedule/ evaluation instead of
 *       /performing/ evaluation; that way multiple output promises
 *       can be fulfilled with only one resulting evaluation.
 */
m4_module
...
function run() {
  ...
  if (data.error) {
    ...
    this.updateEnablement( data.used );
    this.fulfillOutputs( data.result );
  }
}

m4_member(MethodStream, updateEnablement)
function updateEnablement( used ) {
  this.mm.inputs.forEach( function( name ) {
    var vv= this.mm.context[name];
    if (vv instanceof Variable) {
      this.system.setRelevance( vv, this.mm, used[name] );
    }
  }, this );
}

m4_member(MethodStream, fulfillOutputs)
function fulfillOutputs( result ) {
  ...
  for (var i= 0; i < numOutputs; ++i) {
    ...
    if (result[i] instanceof Promise) {
      this.outputPromises[i].add( result[i] );
    }
    else {
      this.outputPromises[i].onCompleted( result[i] );
      // this has the same effect as:
      //     var promise= new Promise( this );
      //     this.outputPromises[i].add( promise );
      //     promise.onCompleted( result[i] );
    }
  }
}
...
function execute( fn, context ) {
  ...
  try {
    ...
    return {used: used, error: e, version: hd.version, complete: true};
  }
}
